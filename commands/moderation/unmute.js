...
module.exports = {
  ...
  let user = message.mentions.users.first() || client.users.cache.get(args[0])
    ...
    if (!membro) return message.channel.send(`Formato inválido. Formato correto: **!unmute @usario motivo.**`)
        ...
        message.channel.send(`Deseja realmente \`desmutar\`  ${user}? Clique no emoji para confirmar.`).then(msg => {
        msg.react("👍")
        ...
        .setThumbnail(avatar)
        .addFields(
        {name: "``Informações do Desmute:``", value: `**Usuário**: ${membro} \n**ID**: ${membro.id} \n**Motivo**: ${motivo} \n **Desmutado por**: ${message.author.username}`
        }
        )
...
msg.awaitReactions({ filter, max: 1, time: 60000, errors: ['time'] })
	.then(collected => canal.send({ embeds: [embed] })).then(collected => membro.roles.remove(role) )
	.catch(collected => {
